...
The deployed technologies includes software implemented in C, {cpp}, eC, Python, Java, and NodeJS. Some of the deployed technologies include Python adapters to software implemented in {cpp}. This variety of programming languages shows that the OGC API specifications are independent of any programming language.
...
There was a https://github.com/opengeospatial/wps-rest-binding/issues/47[suggestion] for highly scalable, chainable and flexible processes centered around 'Collections' as inputs & outputs.
...
There was a discussion about Maps and Tiles being separate building blocks which can be used independently (e.g. together with either Features and/or Coverages), or in combination with each other.

There was a discussion about Maps being in fact a Process taking in one or more collections (which could be collections of vector features, coverages -- collections of gridded cells, or images -- collections of (A)RGB pixels) as inputs, parameters such as styles and producing an (A)RGB image "collection" as an output.
